...
func (uq *UserQuery) QueryRecord() *RankingQuery {
	query := &RankingQuery{config: uq.config}
...
func (uq *UserQuery) WithRecord(opts ...func(*RankingQuery)) *UserQuery {
	query := &RankingQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withRecord; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_record
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_record" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
	...
	if query := uq.withToken; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_token
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_token" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_token" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Token = n
		}
	}
