...
func (aq *AssetQuery) QuerySong() *SongQuery {
...
func (aq *AssetQuery) IDs(ctx context.Context) ([]int, error) {
	var ids []int
...
func (aq *AssetQuery) WithSong(opts ...func(*SongQuery)) *AssetQuery {
	query := &SongQuery{config: aq.config}
...
func (aq *AssetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Asset, error) {
	...
	if query := aq.withSong; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Asset)
		for i := range nodes {
			if nodes[i].song_asset == nil {
				continue
			}
			fk := *nodes[i].song_asset
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "song_asset" returned %v`, n.ID)
			}
			for i := range nodes {
